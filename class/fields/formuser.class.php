	...
	public function render(){
		...
		if ($this->get('showall') && in_array(0, $this->get('selected'))){
			$rtn .= "<li id='".$this->id()."-exmuser-0'>\n
                        <label>
                        <a href='javascript:;' onclick=\"users_field_name='".$this->id()."'; usersField.remove(0);\">&times;</a>
		...
		if (is_array($selected) && !empty($selected) && !(count($selected)==1 && $selected[0]==0)){
			...
			while ($row = $db->fetchArray($result)){
				$rtn .= "<li id='".$this->id()."-exmuser-$row[uid]'>\n
						<label>";
                $rtn .= $this->get('can_change') ? " <a href='#' onclick=\"users_field_name='".$this->id()."'; usersField.remove($row[uid]);\">&times;</a>" : '';
