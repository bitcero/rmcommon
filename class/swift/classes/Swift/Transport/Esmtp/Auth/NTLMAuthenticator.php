...
class Swift_Transport_Esmtp_Auth_NTLMAuthenticator implements Swift_Transport_Esmtp_Authenticator
{
    ...
    public function authenticate(Swift_Transport_SmtpAgent $agent, $username, $password)
    {
        ...
        try {
            ...
            // AUTH NTLM xxxx
            $response = base64_decode(mb_substr(trim($this->sendMessage1($agent)), 4), true);
    ...
    protected function parseMessage2($response)
    {
        ...
        $length = floor(hexdec(mb_substr($responseHex, 28, 4)) / 256) * 2;
        $offset = floor(hexdec(mb_substr($responseHex, 32, 4)) / 256) * 2;
        $challenge = $this->hex2bin(mb_substr($responseHex, 48, 16));
        $context = $this->hex2bin(mb_substr($responseHex, 64, 16));
        $targetInfoH = $this->hex2bin(mb_substr($responseHex, 80, 16));
        $targetName = $this->hex2bin(mb_substr($responseHex, $offset, $length));
        $offset = floor(hexdec(mb_substr($responseHex, 88, 4)) / 256) * 2;
        $targetInfoBlock = mb_substr($responseHex, $offset);
    ...
    protected function readSubBlock($block)
    {
        ...
        $block = mb_substr($block, 0, -8);

        $length = mb_strlen($block);
        $offset = 0;
        $data = [];
        while ($offset < $length) {
            $blockLength = hexdec(mb_substr(mb_substr($block, $offset, 8), -4)) / 256;
            $offset += 8;
            $data[] = $this->hex2bin(mb_substr($block, $offset, $blockLength * 2));
...
. $lmResponse
    ...
    {
        // FIRST PART
        $password = $this->createByte(mb_strtoupper($password), 14, false);
    ...
    protected function getCorrectTimestamp($time)
    {
        ...
        $timestamp = '';
        for ($i = 0; $i < 8; ++$i) {
            $timestamp .= chr(bindec(mb_substr($binary, -(($i + 1) * 8), 8)));
        }
    ...
    protected function createLMv2Password($password, $username, $domain, $challenge, $client)
    {
        ...
        if (mb_strlen($password) <= 15) {
            ...
            $ntml2Hash = $this->md5Encrypt($ntlmHash, $this->convertTo16bit(mb_strtoupper($username) . $domain));
    ...
    protected function createDesKey($key)
    {
        ...
        for ($i = 1; $i < $len; ++$i) {
            ...
            $material[] = str_pad(mb_substr(dechex($v), -2), 2, '0', STR_PAD_LEFT); // cast to byte
        }
        $material[] = str_pad(mb_substr(dechex($this->castToByte(ord($key[6]) << 1)), -2), 2, '0');
    ...
    {
        $length = mb_strlen(bin2hex($value));
    ...
    {
        $length = floor(hexdec(mb_substr($value, 0, 4)) / 256) * 2;
        $offset = floor(hexdec(mb_substr($value, 8, 4)) / 256) * 2;
    ...
    protected function debug($message)
    {
        ...
        $messageId = mb_substr($message, 16, 8);
        echo mb_substr($message, 0, 16) . " NTLMSSP Signature<br>\n";
        ...
        if ('02000000' == $messageId) {
            ...
            $i = 0;
            $data[$i++] = mb_substr($message, 24, 16);
            list($lmLength, $lmOffset) = $this->readSecurityBuffer($data[$i - 1]);

            $data[$i++] = mb_substr($message, 40, 16);
            list($ntmlLength, $ntmlOffset) = $this->readSecurityBuffer($data[$i - 1]);

            $data[$i++] = mb_substr($message, 56, 16);
            list($targetLength, $targetOffset) = $this->readSecurityBuffer($data[$i - 1]);

            $data[$i++] = mb_substr($message, 72, 16);
            list($userLength, $userOffset) = $this->readSecurityBuffer($data[$i - 1]);

            $data[$i++] = mb_substr($message, 88, 16);
            list($workLength, $workOffset) = $this->readSecurityBuffer($data[$i - 1]);

            $data[$i++] = mb_substr($message, 104, 16);
            $data[$i++] = mb_substr($message, 120, 8);
            $data[$i++] = mb_substr($message, $targetOffset, $targetLength);
            $data[$i++] = mb_substr($message, $userOffset, $userLength);
            $data[$i++] = mb_substr($message, $workOffset, $workLength);
            $data[$i++] = mb_substr($message, $lmOffset, $lmLength);
            $data[$i] = mb_substr($message, $ntmlOffset, $ntmlLength);
