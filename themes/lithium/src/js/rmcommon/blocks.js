...
var blocksAjax = {
    ...
    loadBlocker: function(){
        ...
        divBlocker.id = 'settings-blocker';
        document.body.appendChild(divBlocker);
        return divBlocker;
    },

    loadLoading: function(){
        const template = $.templates('#loading-tpl');
        const divLoading = document.createElement('div');
        divLoading.id = 'settings-loading';
        divLoading.innerHTML = template.render();
        document.body.appendChild(divLoading);
        return divLoading;
    },

    unloadLoading: function(){
        $('#loading-blocker').fadeOut('fast', function(){
            $(this).remove();
        });
    },
    ...
    loadForm: function(id,module){
        ...
        const blocker = this.loadBlocker();
        this.loadLoading();
            ...
            $("#settings-loading").fadeIn("fast", function(){
                ...
                $.post('ajax/blocks.php', params, function(data){
                    ...
                    $("#settings-loading").fadeOut("fast", function(){
                        ...
                        div.className = 'cu-box box-primary';
                        div.innerHTML = data.data.content;
                        blocker.appendChild(div);
                        $(div).fadeIn('fast');

                        const triggerTabList = document.querySelectorAll('#settings-form-window .settings-nav button')
    ...
    addMessage: function(message, type){
        ...
        const template = $.templates('#console-log-item');
        const params = {
            message: message,
            type: type
        }

        $("#bk-messages").append(template.render(params));
        $("#console-tab").effect("highlight",{}, 1000);
