...
if (!$xoopsSecurity->checkReferer(1)) {
    response(__('Operation not allowed!', 'rmcommon'), [], 1, 0);
}
...
function insert_block()
{
    ...
    if (!$found) {
        response(__('The specified block does not exists, please verify your selection.', 'rmcommon'), [], 1, 1);
    }
    ...
    $block->setVar('element_type', '' == $bk['type'] ? 'module' : ('theme' == $bk['type'] || 'plugin' == $bk['type'] ? $bk['type'] : 'module'));
    ...
    if (!$block->save()) {
        response(__('Block could not be created. Please try again!', 'rmcommon'), ['error' => $block->errors()], 1, 1);
    }
...
function configure_block()
{
    ...
    if ($block->isNew()) {
        response(__('Specified block does not exists!. Please try again', 'rmcommon'), [], 1, 1);
    }
...
function save_block_config()
{
    ...
    if ($bid <= 0) {
        response(__('You must provide a block ID!', 'rmcommon'), [], 1, 1);
    }

    $block = new RMInternalBlock($bid);
    if ($block->isNew()) {
        response(__('Specified block does not exists!', 'rmcommon'), [], 1, 1);
    }
    ...
    if (!$block->save()) {
        response(sprintf(__('Settings for the block "%s" could not be saved!', 'rmcommon'), $block->getVar('name')), ['error' => $block->errors()], 1, 1);
    }
...
function save_block_position()
{
    ...
    if ($id <= 0) {
        response(__('Specified position is not valid!', 'rmcommon'), [], 1, 1);
    }

    if ('' == $name || '' == $tag) {
        response(__('You must fill name and tag input fields!', 'rmcommon'), [], 1, 1);
    }
    ...
    if ($num > 0) {
        response(__('Already exists another blocks position with same name or tag!', 'rmcommon'), [], 1, 1);
    }
    ...
    if ($pos->save()) {
        response(sprintf(__('Position "%s" was saved successfylly!', 'rmcommon'), $pos->getVar('name')), [], 0, 1);
    } else {
        response(sprintf(__('Position "%s" could not be saved!', 'rmcommon'), $pos->getVar('name')), ['error' => $pos->errors()], 1, 1);
    }
}
...
function save_block_order()
{
    ...
    if (!$xoopsSecurity->check(false, false)) {
        response(__('Blocks order could not be saved!', 'rmcommon'), ['error' => __('Session token expired!', 'rmcommon')], 1, 0);
    }

    if ($pos_id <= 0) {
        response(__('Position not specified!', 'rmcommon'), ['error' => ''], 1, 0);
    }
...
function change_block_visibility($visible)
{
    ...
    if (!$xoopsSecurity->check(false, false)) {
        response(__('Block visibility could not be saved!', 'rmcommon'), ['error' => __('Session token expired!', 'rmcommon')], 1, 0);
    }

    if ($block_id <= 0) {
        response(__('Block identifier not specified!', 'rmcommon'), ['error' => ''], 1, 0);
    }

    $block = new RMInternalBlock($block_id);
    if ($block->isNew()) {
        response(__('Specified block does not exists!', 'rmcommon'), [], 1, 0);
    }
...
function delete_block()
{
    ...
    if (!$xoopsSecurity->check()) {
        response(__('Session token expired!', 'rmcommon'), [], 1, 0);
    }

    $block_id = rmc_server_var($_POST, 'id', 0);

    if ($block_id <= 0) {
        response(__('No block has been specified!', 'rmcommon'), [], 1, 1);
    }
