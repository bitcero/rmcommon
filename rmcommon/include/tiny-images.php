...
if ('' == $action) {
    ...
    if ('tiny' == $type && 'container' != $target) {
    } elseif ('container' != $target && 'external' != $type && 'markdown' != $type) {
    ...
    $links = [
        'none' => ['caption' => __('None', 'rmcommon'), 'value' => ''],
        'file' => ['caption' => __('File URL', 'rmcommon'), 'value' => XOOPS_UPLOAD_URL . '/' . date('Y', $image->getVar('date')) . '/' . date('m', $image->getVar('date')) . '/' . $image->getVar('file')],
    ];
