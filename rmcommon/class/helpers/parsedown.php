...
class RMParsedown
{
    ...
    protected function blockTable($Line, array $Block = null)
    {
        ...
        if (false !== mb_strpos($Block['element']['text'], '|') and '' === rtrim($Line['text'], ' -:|')) {
            $alignments = [];
    ...
    public function line($text)
    {
        ...
        while ($excerpt = strpbrk($text, $this->inlineMarkerList)) {
            ...
            $markerPosition = mb_strpos($text, $marker);
            ...
            $unmarkedText = mb_substr($text, 0, $markerPosition + 1);

            $markup .= $this->unmarkedText($unmarkedText);

            $text = mb_substr($text, $markerPosition + 1);
        }
    ...
    {
        if (false !== mb_strpos($Excerpt['text'], '>') and preg_match('/^<((mailto:)?\S+?@\S+?)>/i', $Excerpt['text'], $matches)) {
            $url = $matches[1];
    ...
    protected function inlineLink($Excerpt)
    {
        ...
        if (preg_match('/^[(]((?:[^ ()]|[(][^ )]+[)])+)(?:[ ]+("[^"]*"|\'[^\']*\'))?[)]/', $remainder, $matches)) {
            ...
            if (preg_match('/^\s*\[(.*?)\]/', $remainder, $matches)) {
                $definition = mb_strlen($matches[1]) ? $matches[1] : $Element['text'];
                $definition = mb_strtolower($definition);

                $extent += mb_strlen($matches[0]);
            } else {
                $definition = mb_strtolower($Element['text']);
            }
    ...
    {
        if ($this->markupEscaped or false === mb_strpos($Excerpt['text'], '>')) {
            return;
        }
    ...
    {
        if (false !== mb_strpos($Excerpt['text'], '>') and preg_match('/^<(\w+:\/{2}[^ >]+)>/i', $Excerpt['text'], $matches)) {
    ...
    protected function li($lines)
    {
        ...
        if (!in_array('', $lines, true) and '<p>' === mb_substr($trimmedMarkup, 0, 3)) {
