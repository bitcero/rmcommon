...
class Swift_Transport_Esmtp_Auth_CramMd5Authenticator implements Swift_Transport_Esmtp_Authenticator
{
    ...
    public function authenticate(Swift_Transport_SmtpAgent $agent, $username, $password)
    {
        try {
            ...
            $challenge = base64_decode(mb_substr($challenge, 4), true);
    ...
    private function _getResponse($secret, $challenge)
    {
        ...
        $k_ipad = mb_substr($secret, 0, 64) ^ str_repeat(chr(0x36), 64);
        $k_opad = mb_substr($secret, 0, 64) ^ str_repeat(chr(0x5C), 64);
