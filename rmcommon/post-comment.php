...
if ('save' == $action) {
    ...
    // Object name
    $object = mb_strtolower(rmc_server_var($_POST, 'object', ''));
    if ('' == trim($object)) {
    ...
    if (is_file(XOOPS_ROOT_PATH . '/modules/' . $object . '/class/' . $object . 'controller.php')) {
        require_once XOOPS_ROOT_PATH . '/modules/' . $object . '/class/' . $object . 'controller.php';
    ...
    if ($cuSettings->comments_notify) {
        ...
        $mailConfig = $configHandler->getConfigsByCat(XOOPS_CONF_MAILER);
    ...
    if ($xoopsUser->isAdmin($comment->getVar('id_obj'))) {
        ...
        $ele->addOption(__('Approved', 'rmcommon'), 'approved', 'approved' == $comment->getVar('status') ? 1 : 0);
        $ele->addOption(__('Unapproved', 'rmcommon'), 'waiting', 'waiting' == $comment->getVar('status') ? 1 : 0);
        $form->addElement($ele);
    }
