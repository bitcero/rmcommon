 ...
 * @return array
 */
function get_authors($mod): array
{
  if ($mod->getInfo('cu_native')) {
    return $mod->getInfo('authors');
  }
...
function module_install_now()
{
  ...
  RMEvents::get()->trigger('rmcommon.installing.module', $mod);
...
function modules_uninstall($dirname)
{
  global $xoopsConfig;
  ...
  if ($module->getVar('dirname') === 'system') {
    ...
    // Get the authors name in a string
    $authorNames= [];
    foreach ($authors as $author) {
      $authorNames[] = $author['name'];
    }

    $msgs[] = '<strong>' . _AUTHOR . ':</strong> ' . htmlspecialchars(implode(', ', $authorNames), ENT_QUOTES);
...
{
  global $xoopsSecurity, $xoopsConfig, $rmTpl, $xoopsDB, $common;
  ...
  RMEvents::get()->trigger('rmcommon.uninstalling.module', $mod);

  $module_log = modules_uninstall($dir);
              ...
              <div class="the-logo">
                <?php if ('' != $mod->getInfo('url')): ?>
                    <a href="modules.php?action=install&amp;dir=<?php echo $mod->getInfo('dirname'); ?>">
                        <img src="<?php echo XOOPS_URL; ?>/modules/<?php echo $mod->getInfo('dirname'); ?>/<?php echo $mod->getInfo('image'); ?>"
                             alt="<?php echo $mod->getInfo('dirname'); ?>">
                    </a>
                <?php else: ?>
                    <img src="<?php echo XOOPS_URL; ?>/modules/<?php echo $mod->getInfo('dirname'); ?>/<?php echo $mod->getInfo('image'); ?>"
                         alt="<?php echo $mod->getInfo('dirname'); ?>">
              ...
              <div class="rmc_mod_info" id="mod-<?php echo $mod->getInfo('dirname'); ?>">
                  ...
                  <table class="table">
                      ...
                      <tr>
                          ...
                          <td>
                              <a target="_blank"
                                 href="<?php echo $mod->getInfo('authorurl'); ?>"><?php echo $mod->getInfo('authorweb'); ?></a>
                          </td>
                      </tr>
                      <tr>
                    ...
                    <?php if ('' != $mod->getInfo('help') && $mod->getInfo('rmnative')): ?>
                        <tr>
                            <td>&nbsp;</td>
                            <td>
                                <strong><a href="<?php echo $mod->getInfo('help'); ?>"
                                           target="_blank"><?php _e('Get Help', 'rmcommon'); ?></a></strong>
                            </td>
                        </tr>
                    <?php endif; ?>
                      <tr>
                          <td colspan="2" class="contact-options text-center">
                            <?php if ($mod->getInfo('authormail')): ?>
                              <?php if ('' != $mod->getInfo('authormail')): ?>
                                    <a target="_blank" href="mailto:<?php echo $mod->getInfo('authormail'); ?>"><span
                                                class="fa fa-envelope"></span></a>
                              <?php endif; ?>
                            <?php endif; ?>
                            <?php if ($mod->getInfo('social')): ?>
                              <?php foreach ($mod->getInfo('social') as $social): ?>
                                    <a target="_blank" href="<?php echo $social['url']; ?>"><span
                                                class="fa fa-<?php echo $social['type']; ?>-square"></span></a>
                              <?php endforeach; ?>
                            <?php endif; ?>
                          </td>
                      </tr>
                      <tr>
                          <td colspan="2" class="text-center">
                              <a href="modules.php?action=install&amp;dir=<?php echo $mod->getInfo('dirname'); ?>"
                                 class="btn btn-success btn-sm"><?php _e('Install', 'rmcommon'); ?></a>
                              <a href="#" onclick="closeInfo();"
                                 class="btn btn-warning btn-sm"><?php _e('Close', 'rmcommon'); ?></a>
                          </td>
                      </tr>
                  </table>
              </div>
